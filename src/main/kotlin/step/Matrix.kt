 ...
 */
fun matrix(regions: List<Region>, alignments: Path, strand: String? = null): List<List<Int>> {
...
fun randomAccessMatrix(regions: List<Region>, alignments: Path, outputFile: Path, batchSize: Int = 10000) {
    ...
    Files.newBufferedWriter(tf).use { tfw ->
        lengths = batches.flatMap {
            val forwardLines = matrix(it, alignments, "forward").map { "[${it.joinToString(",")}]" }
            val reverseLines = matrix(it, alignments, "reverse").map { "[${it.joinToString(",")}]" }
